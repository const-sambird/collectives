...
for operation in "${OPERATIONS[@]}"; do
    for size in "${SIZES[@]}"; do
        for (( i = 0 ; i < 30 ; i++ )); do
            ...
            echo "#SBATCH --nodes=2" >> /scratch/sbird/linked/"$i"/"$operation"_"$size"/"$operation"_"$size".sh
            echo "#SBATCH --ntasks=32" >> /scratch/sbird/linked/"$i"/"$operation"_"$size"/"$operation"_"$size".sh
	    echo "#SBATCH --ntasks-per-node=16" >> /scratch/sbird/linked/"$i"/"$operation"_"$size"/"$operation"_"$size".sh
	    ...
	    echo "#SBATCH --time=00:10:00" >> /scratch/sbird/linked/"$i"/"$operation"_"$size"/"$operation"_"$size".sh
	    echo "#SBATCH --job-name=collective_""$operation"_"$size" >> /scratch/sbird/linked/"$i"/"$operation"_"$size"/"$operation"_"$size".sh
